...
Use this as your live navigation spine if you get lost.

1. **Framework Lab:** show **Frameworks in scope**, then click **Intake -> Review -> Challenge -> Synthesis -> Verdict**.
2. **Framework Findings:** stay on **Verdict** and point to **confidence, citations, scorecards, and risk categories**.
3. **Claude Skills Lab:** show **Framework alone vs framework with Claude skill**, then **Framework still owns / Claude skill now packages**.
4. **Decision Lab:** toggle **Runtime**, **Orchestration**, and **Priority** to show that the recommendation changes with constraints.
5. **Analytics Agent Lab:** select a **product line** and **business question**, then step through the **seven-stage pipeline**.
6. **Analytics Deep Point:** point to **active knowledge** and **Stage ownership**: **Agent**, **MCP/Gateway**, **Data Foundation**.
7. **Close:** scroll to **AgentCore feature cards**: **Persistence**, **Security**, **Compliance**, **Safety**.
